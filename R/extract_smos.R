...
extract_smos <- function(data, lat, lon, save = FALSE,
  ...
  for(i in 1:file_count) {
    ...
    for(k in 1:poi_count) {
      n <- poi_count * (i - 1) + k
      lat_nearest <- which.min(abs(nc_lat - poi@coords[k,2]))
      lon_nearest <- which.min(abs(nc_lon - poi@coords[k,1]))
      sm_data[n,1] <- poi@coords[k,2]
      sm_data[n,2] <- poi@coords[k,1]
      sm_data[n,3] <- freq
      sm_data[n,4] <- orbit
...
set_coords <- function(lat, lon) {
  ...
  poi_coords <- cbind(lon, lat)
  poi <- sp::SpatialPoints(coords = poi_coords)
  sp::proj4string(poi) <- "+proj=longlat +datum=WGS84 +no_defs"
  return(poi)
}
